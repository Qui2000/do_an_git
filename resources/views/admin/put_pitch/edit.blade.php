      ...
      <div class="col-12">
        <form action="" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" placeholder="Mã tài khoản" name="ma_tk" value="{{ $putPitch->ma_tk }}">
          </div>
          <div class="form-group">
            <label for="email">Mã đặt sân:</label>
            <input type="text" class="form-control" placeholder="Mã đặt sân" name="ma_dat_san" value="{{ $putPitch->ma_dat_san }}">
          </div>
          <div class="form-group">
            <label for="pwd">Tên người đặt:</label>
            <input type="text" class="form-control" placeholder="Tên người đặt" name="ten_nguoi_dat" value="{{ $putPitch->ten_nguoi_dat }}">
          </div>
          <div class="form-group">
            <label for="pwd">SDT người đặt:</label>
            <input type="text" class="form-control" placeholder="SDT người đặt" name="sdt_nguoi_dat" value="{{ $putPitch->sdt_nguoi_dat }}">
          </div>
          <div class="form-group">
            <label for="ngay_dat">Ngày đặt:</label>
            <input type="date" class="form-control" placeholder="Ngày đặt" name="ngay_dat" value="{{ $putPitch->ngay_dat }}">
          </div>
          <div class="form-group">
            <label for="dateRequest">Ngày sử dụng</label>
            <input type="date" class="form-control" name="ngay_su_dung" id="dateRequest" value="{{ $putPitch->ngay_su_dung }}">
          </div>
          <div class="form-group">
            <label for="ngay_gio_huy">Ngày hủy sân</label>
            <input type="date" class="form-control" name="ngay_gio_huy" value="{{ $putPitch->ngay_gio_huy }}">
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="khung_gio" name="khung_gio">
                ...
                @foreach (FootballPitch::LIST_TIME_ORDER as $key => $item)
                    ...
                    {{ $key == $putPitch->khung_gio ? "selected" : "" }}
                    >{{ $item }}</option>
                @endforeach
            </select>
          </div>
          ...
          <div class="form-group">
            <label for="pwd">Số lượng nước:</label>
              <input style="width: 100px !important;" id="so_luong" type="number" name="so_luong_dv" value="{{ $putPitch->so_luong_dv }}"
